...
class ICalEvent {
    ...
    start(start) {
        ...
        this.data.start = tools_1.checkDate(start, 'start');
        if (this.data.start && this.data.end && tools_1.toDate(this.data.start).getTime() > tools_1.toDate(this.data.end).getTime()) {
        ...
        this.data.end = tools_1.checkDate(end, 'end');
        if (this.data.start && this.data.end && tools_1.toDate(this.data.start).getTime() > tools_1.toDate(this.data.end).getTime()) {
            const t = this.data.start;
            this.data.start = this.data.end;
            this.data.end = t;
    ...
    recurrenceId(recurrenceId) {
        ...
        this.data.recurrenceId = tools_1.checkDate(recurrenceId, 'recurrenceId');
        return this;
    }
    timezone(timezone) {
    ...
    stamp(stamp) {
        ...
        this.data.stamp = tools_1.checkDate(stamp, 'stamp');
        return this;
    }
    timestamp(stamp) {
        if (stamp === undefined) {
    ...
    repeating(repeating) {
        ...
        if (tools_1.isRRule(repeating) || typeof repeating === 'string') {
            this.data.repeating = repeating;
            return this;
        }
        this.data.repeating = {
            freq: tools_1.checkEnum(types_1.ICalEventRepeatingFreq, repeating.freq)
        };
        if (repeating.count) {
        ...
        if (repeating.until !== undefined) {
            this.data.repeating.until = tools_1.checkDate(repeating.until, 'repeating.until');
        }
        if (repeating.byDay) {
            const byDayArray = Array.isArray(repeating.byDay) ? repeating.byDay : [repeating.byDay];
            this.data.repeating.byDay = byDayArray.map(day => tools_1.checkEnum(types_1.ICalWeekday, day));
        }
        ...
        if (repeating.exclude) {
            ...
            this.data.repeating.exclude = excludeArray.map((exclude, i) => {
                return tools_1.checkDate(exclude, `repeating.exclude[${i}]`);
            });
        }
        if (repeating.startOfWeek) {
            this.data.repeating.startOfWeek = tools_1.checkEnum(types_1.ICalWeekday, repeating.startOfWeek);
        }
        return this;
    }
    summary(summary) {
    ...
    organizer(organizer) {
        ...
        this.data.organizer = tools_1.checkNameAndMail('organizer', organizer);
        return this;
    }
    /**
    ...
    createAttendee(data = {}) {
        ...
        if (typeof data === 'string') {
            data = tools_1.checkNameAndMail('data', data);
        }
    ...
    status(status) {
        ...
        this.data.status = tools_1.checkEnum(ICalEventStatus, status);
        return this;
    }
    busystatus(busystatus) {
        ...
        this.data.busystatus = tools_1.checkEnum(ICalEventBusyStatus, busystatus);
        return this;
    }
    priority(priority) {
    ...
    transparency(transparency) {
        ...
        this.data.transparency = tools_1.checkEnum(ICalEventTransparency, transparency);
        return this;
    }
    created(created) {
        ...
        this.data.created = tools_1.checkDate(created, 'created');
        return this;
    }
    lastModified(lastModified) {
        ...
        this.data.lastModified = tools_1.checkDate(lastModified, 'lastModified');
        return this;
    }
    x(keyOrArray, value) {
        if (keyOrArray === undefined) {
            return tools_1.addOrGetCustomAttributes(this.data);
        }
        if (typeof keyOrArray === 'string' && typeof value === 'string') {
            tools_1.addOrGetCustomAttributes(this.data, keyOrArray, value);
        }
        if (typeof keyOrArray === 'object') {
            tools_1.addOrGetCustomAttributes(this.data, keyOrArray);
        }
        return this;
    }
    /**
    ...
    toJSON() {
        var _a;
        let repeating = null;
        if (tools_1.isRRule(this.data.repeating) || typeof this.data.repeating === 'string') {
            repeating = this.data.repeating.toString();
        }
        else if (this.data.repeating) {
            repeating = Object.assign({}, this.data.repeating, {
                until: tools_1.toJSON(this.data.repeating.until),
                exclude: (_a = this.data.repeating.exclude) === null || _a === void 0 ? void 0 : _a.map(d => tools_1.toJSON(d)),
            });
        }
        return Object.assign({}, this.data, {
            start: tools_1.toJSON(this.data.start) || null,
            end: tools_1.toJSON(this.data.end) || null,
            recurrenceId: tools_1.toJSON(this.data.recurrenceId) || null,
            stamp: tools_1.toJSON(this.data.stamp) || null,
            created: tools_1.toJSON(this.data.created) || null,
            lastModified: tools_1.toJSON(this.data.lastModified) || null,
            repeating,
            x: this.x()
        });
    }
    ...
    toString() {
        ...
        g += 'DTSTAMP:' + tools_1.formatDate(this.calendar.timezone(), this.data.stamp) + '\r\n';
        if (this.data.allDay) {
            g += 'DTSTART;VALUE=DATE:' + tools_1.formatDate(this.calendar.timezone(), this.data.start, true) + '\r\n';
            if (this.data.end) {
                g += 'DTEND;VALUE=DATE:' + tools_1.formatDate(this.calendar.timezone(), this.data.end, true) + '\r\n';
            }
            g += 'X-MICROSOFT-CDO-ALLDAYEVENT:TRUE\r\n';
            g += 'X-MICROSOFT-MSNCALENDAR-ALLDAYEVENT:TRUE\r\n';
        }
        else {
            g += tools_1.formatDateTZ(this.timezone(), 'DTSTART', this.data.start, this.data) + '\r\n';
            if (this.data.end) {
                g += tools_1.formatDateTZ(this.timezone(), 'DTEND', this.data.end, this.data) + '\r\n';
            }
        }
        // REPEATING
        if (tools_1.isRRule(this.data.repeating) || typeof this.data.repeating === 'string') {
            ...
            if (this.data.repeating.until) {
                g += ';UNTIL=' + tools_1.formatDate(this.calendar.timezone(), this.data.repeating.until);
            }
            if (this.data.repeating.byDay) {
            ...
            if (this.data.repeating.exclude) {
                if (this.data.allDay) {
                    g += 'EXDATE;VALUE=DATE:' + this.data.repeating.exclude.map(excludedDate => {
                        return tools_1.formatDate(this.calendar.timezone(), excludedDate, true);
                    }).join(',') + '\r\n';
                }
                else {
                    g += 'EXDATE';
                    if (this.timezone()) {
                        g += ';TZID=' + this.timezone() + ':' + this.data.repeating.exclude.map(excludedDate => {
                            ...
                            return tools_1.formatDate(this.timezone(), excludedDate, false, true);
                        }).join(',') + '\r\n';
                    }
                    else {
                        g += ':' + this.data.repeating.exclude.map(excludedDate => {
                            return tools_1.formatDate(this.timezone(), excludedDate);
                        }).join(',') + '\r\n';
                    }
                }
            }
        ...
        if (this.data.recurrenceId) {
            g += tools_1.formatDateTZ(this.timezone(), 'RECURRENCE-ID', this.data.recurrenceId, this.data) + '\r\n';
        }
        // SUMMARY
        g += 'SUMMARY:' + tools_1.escape(this.data.summary) + '\r\n';
        // TRANSPARENCY
        if (this.data.transparency) {
            g += 'TRANSP:' + tools_1.escape(this.data.transparency) + '\r\n';
        }
        // LOCATION
        if ((_a = this.data.location) === null || _a === void 0 ? void 0 : _a.title) {
            if (this.data.location.radius && this.data.location.geo) {
                g += 'LOCATION:' + tools_1.escape(this.data.location.title +
                    (this.data.location.address ? '\n' + this.data.location.address : '')) + '\r\n';
                g += 'X-APPLE-STRUCTURED-LOCATION;VALUE=URI;' +
                    (this.data.location.address ? 'X-ADDRESS=' + tools_1.escape(this.data.location.address) + ';' : '') +
                    'X-APPLE-RADIUS=' + tools_1.escape(this.data.location.radius) + ';' +
                    'X-TITLE=' + tools_1.escape(this.data.location.title) +
                    ':geo:' + tools_1.escape((_b = this.data.location.geo) === null || _b === void 0 ? void 0 : _b.lat) + ',' + tools_1.escape((_c = this.data.location.geo) === null || _c === void 0 ? void 0 : _c.lon) + '\r\n';
            }
            else {
                g += 'LOCATION:' + tools_1.escape(this.data.location.title) + '\r\n';
            }
            if (this.data.location.geo) {
                g += 'GEO:' + tools_1.escape((_d = this.data.location.geo) === null || _d === void 0 ? void 0 : _d.lat) + ';' + tools_1.escape((_e = this.data.location.geo) === null || _e === void 0 ? void 0 : _e.lon) + '\r\n';
            }
        }
        // DESCRIPTION
        if (this.data.description) {
            g += 'DESCRIPTION:' + tools_1.escape(this.data.description.plain) + '\r\n';
            // HTML DESCRIPTION
            if (this.data.description.html) {
                g += 'X-ALT-DESC;FMTTYPE=text/html:' + tools_1.escape(this.data.description.html) + '\r\n';
            }
        }
        // ORGANIZER
        if (this.data.organizer) {
            g += 'ORGANIZER;CN="' + tools_1.escape(this.data.organizer.name) + '"';
            if (this.data.organizer.email && this.data.organizer.mailto) {
                g += ';EMAIL=' + tools_1.escape(this.data.organizer.email);
            }
            if (this.data.organizer.email) {
                g += ':mailto:' + tools_1.escape(this.data.organizer.mailto || this.data.organizer.email);
            }
            g += '\r\n';
        }
        // ATTENDEES
        ...
        // URL
        if (this.data.url) {
            g += 'URL;VALUE=URI:' + tools_1.escape(this.data.url) + '\r\n';
        }
        // STATUS
        if (this.data.status) {
        ...
        // CUSTOM X ATTRIBUTES
        g += tools_1.generateCustomAttributes(this.data);
        // CREATED
        if (this.data.created) {
            g += 'CREATED:' + tools_1.formatDate(this.calendar.timezone(), this.data.created) + '\r\n';
        }
        // LAST-MODIFIED
        if (this.data.lastModified) {
            g += 'LAST-MODIFIED:' + tools_1.formatDate(this.calendar.timezone(), this.data.lastModified) + '\r\n';
        }
        g += 'END:VEVENT\r\n';
        return g;
    }
