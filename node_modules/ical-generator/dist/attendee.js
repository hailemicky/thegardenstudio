...
class ICalAttendee {
    ...
    role(role) {
        ...
        this.data.role = tools_1.checkEnum(ICalAttendeeRole, role);
        return this;
    }
    rsvp(rsvp) {
        if (rsvp === undefined) {
    ...
    status(status) {
        ...
        this.data.status = tools_1.checkEnum(ICalAttendeeStatus, status);
        return this;
    }
    type(type) {
        if (type === undefined) {
        ...
        this.data.type = tools_1.checkEnum(ICalAttendeeType, type);
        return this;
    }
    delegatedTo(delegatedTo) {
        ...
        if (typeof delegatedTo === 'string') {
            this.data.delegatedTo = new ICalAttendee(tools_1.checkNameAndMail('delegatedTo', delegatedTo), this.event);
        }
    ...
    delegatedFrom(delegatedFrom) {
        ...
        if (!delegatedFrom) {
            ...
            this.data.delegatedFrom = new ICalAttendee(tools_1.checkNameAndMail('delegatedFrom', delegatedFrom), this.event);
        }
    ...
    toString() {
        ...
        // CN / Name
        if (this.data.name) {
            g += ';CN="' + tools_1.escape(this.data.name) + '"';
        }
        // EMAIL
        if (this.data.email && this.data.mailto) {
            g += ';EMAIL=' + tools_1.escape(this.data.email);
        }
        // CUSTOM X ATTRIBUTES
        if (this.data.x.length) {
            g += ';' + this.data.x
                .map(([key, value]) => key.toUpperCase() + '=' + tools_1.escape(value))
                .join(';');
        }
        g += ':MAILTO:' + tools_1.escape(this.data.mailto || this.data.email) + '\r\n';
        return g;
    }
}
